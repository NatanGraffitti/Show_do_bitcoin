import random														#import para embaralhar as perguntas																	# import 
...
def atualiza_premio(guarda_nivel):												#determina o valor do premio atual e aumenta o premio a cada pergunta correta
    ...
    if 0 <= guarda_nivel < len(valor_premios):											#verifica se o nivel esta dentro dos limites da lista, retorna o premio que corresponde ao nivel e se nao, retorna nivel invalido
...
def get_questions_from_api(token, qtd_questions, difficulty=None):									#busca perguntas na API com base no token, quantidade e nivel de dificuldade
    ...
    params = {
        ...
        'category': category, # Pode ser ajustado se a API tryvia tiver outras categorias
        'type': 'multiple',
        'token': token
    }
    ...
    try:
        ...
        data = response.json()
        if data.get("response_code") != 0:
            # Response code 1 significa "No results", 2 é "Invalid Parameter", etc.
            # Se for "No results", não é necessariamente um erro fatal, mas significa que não há perguntas.
